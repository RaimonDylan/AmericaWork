...
-- Procédures
--
CREATE PROCEDURE `closeAdventure` ()  NO SQL
    DETERMINISTIC
BEGIN
    UPDATE launchadventure SET inProgress = 0 WHERE inProgress = 1;
END$$

CREATE PROCEDURE `getAllQuestions` ()  NO SQL
    DETERMINISTIC
BEGIN
    SELECT libelle, 
...
CREATE PROCEDURE `getResultsVote` (IN `_id` INT)  NO SQL
    DETERMINISTIC
BEGIN
...
-- Fonctions
--
CREATE FUNCTION `isAdventure` () RETURNS TINYINT(1) NO SQL
    DETERMINISTIC
BEGIN
...
CREATE FUNCTION `isQuestion` () RETURNS TINYINT(1) NO SQL
BEGIN
    DECLARE lvl integer(1);
...
CREATE FUNCTION `startAdventure` (`_id` INT) RETURNS INT(11) BEGIN
...
CREATE FUNCTION `startQuestion` (`_idA` INT, `_idQ` INT) RETURNS INT(11) BEGIN
    DECLARE lvl integer(11);
